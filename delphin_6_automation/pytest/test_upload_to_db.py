...
import delphin_6_automation.nosql.db_templates as db_temp
...
def test_upload_project_1():
    ...
    test_doc = db_temp.Delphin.objects(id=id_).first()
    test_dict = test_doc.to_mongo()
    test_dict.pop('_id')
    test_dict.pop('added_date')

    source_dict = db_temp.Delphin.objects(id='5a5479095d9460327c6970f0').first().to_mongo()
    source_dict.pop('_id')
